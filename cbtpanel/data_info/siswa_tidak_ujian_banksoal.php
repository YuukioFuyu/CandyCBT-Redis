	...
	<div class='col-md-12'>
		<div class='box box-solid'>
			<div class='box-header with-border'>
				<h3 class='box-title'>Siswa Tidak Mengikuti Ujian Per Jadwal</h3>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			$(document).on('click','#cari_nilai',function(){
				...
				$.ajax({
					type: 'POST',
					url: 'c_aksi.php?adm=cek_siswabanksoal',
					data:{kelas:kelas,ujian:ujian,jrs:jrs},
					dataType : 'json',
					success: function(data){
						var data2 = JSON.stringify(data);
						localStorage.setItem('tidakujianbanksoal', data2);
						location.reload();
		...
		var dataSet;
		try{
			dataSet = JSON.parse(localStorage.getItem('tidakujianbanksoal')) || [];
		} catch (err) {
			dataSet = [];
		}
		var dataMapel = [];
		$.each(dataSet, function(index, objek){
			if(objek.status==1){ var status="Belum Ujian" }
			var option = '<tr><td>'+objek.username+'</td><td>'+objek.nama_siswa+
			'</td><td>'+objek.kelas+'</td>'+
			'<td>'+objek.sesi+'</td>'
			+'<td>'+objek.ruang+'</td>'
			+'<td>'+objek.server+'</td>'
			+'<td>'+objek.agama+'</td>'
			+'<td>'+objek.paket+'</td>'
			+'<td>'+objek.statusSiswa+'</td>'
