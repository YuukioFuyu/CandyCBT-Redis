...
if($token == $token1) {
    ...
    if (isset($_FILES['file']['name'])) {
        ...
        for ($i = 1; $i < count($sheetData); $i++) {
            ...
            $firt_nama = addslashes($firt_nama);
            $paketsoal = $sheetData[$i]['15'];
            ...
            if ($nama <> '') {
                $exec = mysqli_query($koneksi, "INSERT INTO siswa (id_siswa,id_kelas,idpk,nis,no_peserta,nama,level,sesi,ruang,username,password,foto,server,agama,firt_nama,soalPaket) VALUES ('$id_siswa','$kelas','$pk','$nis','$no_peserta','$nama','$level','$sesi','$ruang','$username','$password','$foto','$server','$agama','$firt_nama','$paketsoal')");
                ($exec) ? $sukses++ : $gagal++;
            }
        }
