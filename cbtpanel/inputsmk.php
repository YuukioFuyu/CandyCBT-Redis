						...
						<div class='btn-group'>
							...
							if ($jenis == '1' or $jenis == '3') {
								for ($i = 1; $i <= $mapel['jml_soal']; $i++) {
									$ceksoaldb = mysqli_query($koneksi, "SELECT nomor FROM soal WHERE id_mapel='$id_mapel' AND nomor='$i' AND jenis='1' OR jenis='3'");
									...
									if(!empty($pagingNoSoal['jenis'])){$noJenis = $pagingNoSoal['jenis']; }else{ $noJenis =1 ; }
