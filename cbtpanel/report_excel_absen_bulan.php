...
if($token == $token1) {
  ...
  $kelas1 = $kelass->fetch_array();
	
	//mata pelajaran dan kelas
	$mapel = $db->getMapelAbsen($mapel);
	$mapel1 = $mapel->fetch_array();
	if($_SESSION['level']=='admin'){ 	$guru = fetch($koneksi, 'pengawas',array('id_pengawas'=>$mapel1['id_pengawas'])); }
	else{ $guru = fetch($koneksi, 'pengawas',array('id_pengawas'=>$_SESSION['id_pengawas'])); }
	...
	//proses excel -----------------------------------------------------------
	$file = "Absensi_Total_" . $tahun. "_".$bulan."_".$mapel1['amNamaMapel'];
...
<div class="landscape">
	...
	<table class='detail' border="0">
		<tr>
			<td colspan='3'>TAHUN</td><td colspan='12'><span > : &nbsp; <?= $tahun ?></span></td>
			
			
		</tr>
		<tr>
			<td colspan='3'>BULAN</td><td colspan='12'><span > : &nbsp; <?= strtoupper(bulanIndo($bulan)) ?></span></td>
			
		</tr>
		<tr>
			<td width="170" colspan='3'>MATA PELAJARAN</td><td colspan='12'><span > : &nbsp;<?= $mapel1['amNamaMapel'];?></span></td>
			
		</tr>
		<tr>
			<td colspan='3' >KELAS</td><td colspan='12'><span > : &nbsp;<?= $kelas1['id_kelas'];?></span></td>
	...
	<table class='it-grid it-cetak' width='100%' border="1">
		...
		<tbody>
			...
			<tr>
				...
				<?php 
				for ($i=1; $i <=31 ; $i++) { 
					echo '<td width="10%"" align="center">'.$value['tgl'.$i].'</td>';
				} ?>
	...
	<div style='padding-left: 50px;'>
		...
		<table border='0' width='100%'>
			...
			<tr>
				<td ></td>
				<td ><?= $setting['kecamatan'] ?>, <?= $db->tgl_indo(date('Y-m-d')); ?></td>
			</tr>
