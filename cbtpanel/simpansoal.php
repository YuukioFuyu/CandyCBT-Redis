...
if (isset($_POST['mapel'])) {
	$redis->RedisDelKey('json_soal');
	$allowed_ext = array('jpg', 'png', 'jpeg', 'gif', 'mp3', 'wav','PNG','JPG');
	$nomor = $_POST['nomor'];
	$jenis = $_POST['jenis'];
	...
	if ($jenis == '4') { #soal cekbok multipel chois
		$jumsoal = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM soal WHERE id_mapel='$id_mapel' AND  nomor='$nomor' AND jenis='4'"));
		$soalQ = mysqli_query($koneksi, "SELECT * FROM soal WHERE id_mapel='$id_mapel' AND  nomor='$nomor' AND jenis='4'");
	}
	if ($jenis == '2') { #soal esai
	...
	if ($jenis == '1' or $jenis == 3 or $jenis == 4) {
		...
		if ($jumsoal == 0) {
			if ($setting['jenjang'] == 'SMK') {
				$exec = mysqli_query($koneksi, "INSERT INTO soal (id_mapel,nomor,soal,jenis,pilA,pilB,pilC,pilD,pilE,jawaban,file,file1,fileA,fileB,fileC,fileD,fileE) VALUES ('$id_mapel','$nomor','$isi_soal','$jenis','$pilA','$pilB','$pilC','$pilD','$pilE','$jawaban','$urlx','$filex1','$filexA','$filexB','$filexC','$filexD','$filexE')");
			} else {
				$exec = mysqli_query($koneksi, "INSERT INTO soal (id_mapel,nomor,soal,jenis,pilA,pilB,pilC,pilD,jawaban,file,file1,fileA,fileB,fileC,fileD) VALUES ('$id_mapel','$nomor','$isi_soal','$jenis','$pilA','$pilB','$pilC','$pilD','$jawaban','$urlx','$filex1','$filexA','$filexB','$filexC','$filexD')");
			}
		} else {
			if ($setting['jenjang'] == 'SMK') {
				$exec = mysqli_query($koneksi, "UPDATE soal SET soal='$isi_soal',pilA='$pilA',pilB='$pilB',pilC='$pilC',pilD='$pilD',pilE='$pilE',jawaban='$jawaban',file='$urlx',file1='$filex1',fileA='$filexA',fileB='$filexB',fileC='$filexC',fileD='$filexD',fileE='$filexE' WHERE id_mapel='$id_mapel' AND nomor='$nomor' AND jenis='$jenis'");
				echo mysqli_error($koneksi);
			} else {
				$exec = mysqli_query($koneksi, "UPDATE soal SET soal='$isi_soal',pilA='$pilA',pilB='$pilB',pilC='$pilC',pilD='$pilD',jawaban='$jawaban',file='$urlx',file1='$filex1',fileA='$filexA',fileB='$filexB',fileC='$filexC',fileD='$filexD' WHERE id_mapel='$id_mapel' AND nomor='$nomor' AND jenis='$jenis'");
			}
		}
	}
	if ($jenis == '2') {
