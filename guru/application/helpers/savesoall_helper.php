...
function save_soal($id) {
  ...
  foreach ($sqlcek as $r) {
    $noopsi = $no++;
    $s_soal = $ci->savesoal->select_savsoft_qbank($noopsi)->row_array();
    $soal_tanya = $s_soal['question'];
    $l_soal = $s_soal['lid'];
    $c_id = $s_soal['cid'];
    $g_soal = $s_soal['description'];
    $g_soal = str_replace(" ", "", $g_soal);
    $smin = $ci->savesoal->select_savsoft_options($noopsi);
    foreach ($smin as $hmin) {
      $min_op = $hmin['mini'];
    }

    $sqlopc = $ci->savesoal->select_savsoft_options2($noopsi,$min_op)->row_array();
    $ropc = $sqlopc;
    $opj1 = $ropc['q_option'];
    ...
    $dele = $ci->savesoal->hapus_savsoft_options2($noopsi,$min_op);
    //---------------------------------------------------------------------------------------
    $smin = $ci->savesoal->select_savsoft_options($noopsi);
    foreach ($smin as $hmin) {
      $min_op = $hmin['mini'];
    ...
    $sqlopc = $ci->savesoal->select_savsoft_options2($noopsi,$min_op)->row_array();
    $rubah = $ci->savesoal->select_savsoft_options3($noopsi);
