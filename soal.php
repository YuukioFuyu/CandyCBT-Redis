<?php
	...
	//get soal ujian ------------------------------------
		...
		foreach ($pengacakq as $dataacak) {
			$pengacak = explode(',', $dataacak->id_soal);//pecah ke array
		}
		...
		#ambil jenis soal dari tabel soal
		$jensilSoalDb = $getSoal[0]->jenis;
...
<div class="bagian_sola" >
	...
	<h3 class='box-title'><span class='btn hidden-xs bg-gray active'><?= $getBlok['blok']; 	?>  SOAL NO </span> <span class='btn bg-green' id='displaynum'><b><?= $no_next; ?></b></span></h3>
	<div class='btn-group'>
				...
				<div class="col-md-12 col-xm-6">
					...
					<div class='col-md-12'>
						<table width='100%' class='table tabelpg'>
							...
							<?php #jika jenis soalnya benar salah 
							if($jensilSoalDb==3) : ?>
								<!-- OPSI A -->
								<tr>
									<td class="width_td">
										<label class="container">
											<input class="jawab" id="radio-<?= $data->id_soal ?>-A" type="radio" value="A" name='jawab<?= $data->id_soal ?>' id='A'>
											<span class="checkmark"><center >T</center></span>
										</label>
									</td>
									<td>
										<span class='soal' ><?= $data->pilA ?></span>
								...
								<tr>
									<td class="width_td">
										<label class="container">
											<input class="jawab" id="radio-<?= $data->id_soal ?>-B" type="radio" value="B" name='jawab<?= $data->id_soal ?>' id='B'>
											<span class="checkmark"><center>F</center></span>
										</label>
									</td>
									<td> 
										<span class='soal' ><?= $data->pilB ?></span>
							...
							<?php #jika jenis soalnya multicois
							if($jensilSoalDb==4) : 
								include('template/opsi_soal_multi.php');
							endif;  ?>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
      	...
      	?>
        <button onclick="loadsoalpg(<?= $noo++; ?>,1)" class='daftarsoal<?= $valuee; ?> btn btn-app' id="sola-id-daftar-<?= $valuee; ?>" data-id="<?= $valuee; ?>" style="min-width:50px;height:50px;border-radius:10px;border:solid black;font-size:medium;" ><?= $no++;?>
				<span id='jawabtemp-daftar<?= $valuee; ?>' class='badge bg-green' style="font-size:medium"></span></button>
							...
							$.each(jawabs_cek2, function(index, obje){
								...
								<?php //tombol ragu pada daftar soal ?>
									...
									if(idragu > 0 ){
										...
										style = 'background:#ffbe08;min-width:50px;height:50px;border-radius:10px;border:solid black;font-size:medium;"';
									}
...
<?php //----------------tombol finis-------------------- ?>
	$("#finis").click(function(){
		...
		$.ajax({
			type: 'POST',
			url:'../../c_jawaban.php?siswa=kirim_jawaban',
			data: {nilai:send},
			beforeSend: function() {
